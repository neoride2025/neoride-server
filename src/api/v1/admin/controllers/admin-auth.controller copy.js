...
const { COOKIE_OPTIONS, MAX_SESSION_DAYS } = require("../utils/util-config");

const config = require("../../../../config/jwt.config");
const authService = require("./admin-auth.service");
const { validateLoginForm } = require("./admin-auth.validator");
...
exports.login = async (req, res, next) => {
  try {
    ...
    if (!user) {
      const err = new Error();
      err.statusCode = 400;
      err.message = MSG.AUTHENTICATION.USER_NOT_FOUND;
      throw err;
    }
    ...
    if (!match) {
      const err = new Error();
      err.statusCode = 401;
      err.message = MSG.AUTHENTICATION.INVALID_CREDENTIALS;
      throw err;
    }
...
exports.logout = async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      status: 200,
      message: MSG.AUTHENTICATION.LOGOUT_SUCCESS,
    });
  } catch (err) {
    next(err);
  }
